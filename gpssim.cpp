...
*/
    ...
    while (1)
    {
        ...
        iq_buff = sample_buffers[write_buff_pos].buff_ptr;
		for (i=0; i<MAX_CHAN; i++)
		{
			if (chan[i].prn>0)
			{
		...
		fflush(stdout);
        sample_buffers[write_buff_pos].full=1;
        write_buff_pos = (write_buff_pos+1) % BUFFER_COUNT;
        if(!holdLocation)
        {
            iumd++;
            running_time++;
...
void GpsSimThread::set_stop(bool v)
{
    stop = v;
    if(stop)
    {
...
void RadioThread::run()
{
    ...
    while(1)
    {
        ...
        {
            timestamp++;
            int r = radio_transmit(sample_buffers[read_buff_pos].buff_ptr, iq_buff_size);
            if(r != RADIO_NOERROR)
            {
                if(timestamp > error_time + 60)
                {
...
void RadioThread::set_stop(bool v)
{
    stop = v;
}
